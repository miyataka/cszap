...
| **SJF**（shortest job first） | 実行時間が最短のもの | **平均待ち時間を最小化**（理論的最適。毎回いちばん短いものを選ぶ貪欲法 → [AL-Strategies §4](../AL/AL-Strategies.md#greedy)） | 実行時間は事前に分からない（予測が必要）。長いジョブが**飢餓**しうる |
...
- 定石：タイマ要求を**満了時刻順のデータ構造**（ソート済みリスト・タイマホイール・優先度キュー（ヒープ実装 → [AL-Foundational §3](../AL/AL-Foundational.md#tree)））で持ち、ハードウェアタイマは**最も近い満了時刻**にセットする。割り込みが来たら満了分を処理し、次の満了時刻で再セット。
