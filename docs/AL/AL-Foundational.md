...
中心の問いは「**解きたい問題**に、どの入れ物（データ構造）とどの手続き（アルゴリズム）を当てるか」。問題の性質からデータ構造・アルゴリズムへ矢印を引くと、本ユニットの節がそのまま「道具箱の引き出し」として見えてくる。
...
graph LR
  ...
  subgraph ALGO["アルゴリズム"]
    BS[二分探索]
    SORT[ソート]
    SP["最短経路<br/>Dijkstra・Floyd"]
    MST["最小全域木<br/>Prim・Kruskal"]
    TOPO[トポロジカルソート]
    STRM["文字列照合<br/>Boyer-Moore・KMP"]
  end

  Q -->|途中の挿入・削除を保ちたい| LL
  Q -->|キーで平均一定時間の検索・登録| HASH
  Q -->|優先度順に取り出したい| HEAP
  Q -->|階層・ネットワークを表したい| TREE
  Q -->|ソート済みから探したい| BS
  Q -->|データを並べ替えたい| SORT
  Q -->|文字列パターンを探したい| STRM
  TREE -->|頂点間の最短経路が欲しい| SP
  TREE -->|全頂点を最小コストでつなぎたい| MST
  TREE -->|依存順に並べたい| TOPO
```

**図の読み方**

- 中心の問い「解きたい問題は?」から、まず**データ構造**（入れ物）と**アルゴリズム**（手続き）のどちらを選ぶ場面かに分かれる。
- 連結リスト・ハッシュ表・ヒープ・木は [§1 線形データ構造](#linear)・[§2 連想・集合の構造](#assoc-set)・[§3 グラフと木](#tree) に対応。
- 二分探索は [§4 探索アルゴリズム](#search)、並べ替えは [§5 ソートアルゴリズム](#sort)。木・グラフから伸びる最短経路・最小全域木・トポロジカルソートは [§6 グラフアルゴリズム](#graph-algorithms)、文字列照合は [§7 文字列照合](#matching)。
- ヒープを優先度付きキューとして使う実装は、そのまま [OS-Scheduling §2](../OS/OS-Scheduling.md#policies) の優先度スケジューラで動いている。
- どの構造・アルゴリズムを選んだ後も、それが**どの設計パラダイムに属するか**は [AL-Strategies](AL-Strategies.md#overview) で整理し直す。
...
| 6 | 計算効率**以外**にアルゴリズム選択を左右する要因——実装にかかる時間、保守性、入力データに含まれるアプリケーション固有のパターンの利用——を説明できる。 | [§5](#sort) |
