...
| 4 | **循環待機** (circular wait) | 待ちの連鎖が輪になる。「どのスレッドがどの資源を待つか」を有向グラフ（資源割り当てグラフ）に描くと**閉路がある**状態にあたり、デッドロックの検出はそのグラフに閉路がないか——すなわち [AL-Foundational §6](../AL/AL-Foundational.md#graph-algorithms) のトポロジカルソートが成立する DAG（→ [§3](../AL/AL-Foundational.md#tree)）かどうか——を調べることになる | **資源に全順序を付け、順番にしか獲得させない** |
