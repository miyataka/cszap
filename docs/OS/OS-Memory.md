...
```mermaid
graph TD
  Q["中心的な問い<br/>仮想アドレスはどう物理アドレスになるか"]

  Q --> VA["CPUが出す仮想アドレス"]
  VA --> TLB{"TLB<br/>ヒット?"}
  TLB -->|"ヒット"| PA["物理アドレス"]
  TLB -->|"ミス"| PT["ページテーブルを引く"]
  PT -->|"存在する"| PA
  PT -->|"存在しない"| PF["ページフォルト"]
  PF -->|"空きフレームなし"| REPL["ページ置換<br/>FIFO・LRU・クロック"]
  REPL -->|"フレーム不足が慢性化"| THRASH["スラッシング"]

  subgraph ALLOCSIDE["脇道：アロケータ"]
    ALLOC["アロケータ<br/>フリーリスト・スラブ・アリーナ"]
  end

  subgraph CROSS["脇道：横断的関心"]
    NOTE["キャッシュ階層とコヒーレンス<br/>投機実行対策 (KPTI等)<br/>サンドボックス・分離"]
  end
...
- CPU が出す仮想アドレス→TLB ヒット/ミス→ページテーブル→物理アドレス の流れ → [§1 物理メモリ・アドレス変換・MMU](#address-translation)。
- ページテーブルに無ければページフォルト→ページ置換→スラッシング → [§4 ページング・ページ置換・スラッシング](#paging)。
- アロケータは「ページの中の使い方」としてプロセス側から仮想アドレスにつながる → [§5 アロケータ](#allocators)。
- 物理アドレス確定後も横断的関心として効く：キャッシュ階層とコヒーレンス（[§2 メモリ階層とキャッシュの OS への影響](#memory-hierarchy)）、投機実行対策（[§6 キャッシュコヒーレンスと投機実行脆弱性対策のフラッシュ](#cache-speculative)）、サンドボックス・分離（[§7 メモリ管理のセキュリティ](#memory-security)）。
- スラッシングへの対処は [OS-Scheduling §2 スケジューラとポリシー](OS-Scheduling.md#policies) の中期スケジューラ、投機実行対策の実例は [OS-Protection §3 実在 OS の重大脆弱性レビュー](OS-Protection.md#real-vulnerabilities)。
...
- **ページ置換**: 空きフレームが尽きたら誰かを追い出す。**FIFO**（単純だが古参が重要でも追い出す。読み込んだ順のキューをそのまま使う → [AL-Foundational §1](../AL/AL-Foundational.md#linear) のキュー）、**LRU**（最近使っていないものを追い出す。理想に近いが厳密な実装は高価）、**クロック**（参照ビットで LRU を近似。実用の定番）。
...
| 技法 | 仕組み | 得失 |
|---|---|---|
| **フリーリスト** | 空き塊を連結リストで管理し、first-fit / best-fit で探す（→ [AL-Foundational §1](../AL/AL-Foundational.md#linear) の連結リスト） | 汎用だが探索コストと**断片化**が課題 |
| **サイズクラス** | よく使うサイズ別に空きリストを分けておく | 探索が O(1) に（→ [AL-Strategies §6](../AL/AL-Strategies.md#space-time-tradeoffs) の空間と時間のトレードオフ）。クラス内の余りは内部断片化 |
