...
- **分散処理**: 複数台のコンピュータで処理を分担する方式。機能拡張は容易だが、保守・セキュリティ・運用管理は複雑になる。ただし「1台が故障しても処理を継続できる」のは分散させるだけで自動的に得られる性質ではなく、冗長化・データの複製・処理の自動切り替えや必要な予備容量の確保といった設計があって初めて実現する。単に別々の必須機能を複数台に分担させただけの構成では、そのうち1台の故障でシステム全体が止まることもある。

近年はインターネット経由で利用するクラウドコンピューティングが普及し、利用者からは集中処理か分散処理かの違いが見えにくくなっている。ただしそれは利用者側から意識しにくくなっただけで、可用性をどう確保するかという設計上の重要性が薄れたわけではない。
...
**暗号化**とは元のデータ（平文）を容易に内容が分からない別のビット列（暗号文）に変換する技術、**復号**は暗号文を平文に戻すこと。暗号化単体が主に提供するのは盗聴・データ流出に対する**機密性**であり、改ざんの検出には認証付き暗号（AEAD）や MAC・デジタル署名、なりすましへの対処には別途の認証の仕組みが必要になる。
...
内部ネットワークと外部ネットワーク（インターネット）の間に置かれ、双方との通信を制御する別セグメントを**DMZ（DeMilitarized Zone）**という。内部・外部のどちらからも完全に切り離された区域ではなく、外部に公開する必要のあるサーバなどをここに置くことで、そのサーバが侵害された場合でも内部ネットワークへの被害の波及を抑えるための設計上の工夫であり、被害が及ばないことを保証するものではない。
...
アクセス制御方式には、ユーザー自身が制御を行う**任意アクセス制御**（自由度は高いがセキュリティ面で課題）、管理者だけがルールを変更できる**強制アクセス制御**（一般に任意アクセス制御より安全）、「役割（ロール）」に権限をまとめて割り当て、利用者をその役割に所属させることでアクセス権を管理する**役割ベースのアクセス制御（RBAC）**がある。RBAC は管理者権限の委譲に限らず、一般利用者の閲覧・編集権限の管理にも広く使われる。フォルダ・ファイル単位でも「読み取り」「追加」「修正」「削除」などのアクセス権を、ユーザーごと・グループごとに設定できる。
...
[§4](#modeling) の**要件定義**で「利用者から何が求められるか」を整理・合意し、その結果に基づいて画面・帳票・外部インタフェースなど利用者から見える部分を具体化する工程を**外部設計**、それをどうプログラムとして記述するかを考える工程を**内部設計**と呼ぶ。
...
プログラミング言語にはデータの扱いに適したもの、機械の制御に適したもの、グラフィックスに強いものなど特性があり、作ろうとするものに合った言語を選ぶことが大切。なお、コンパイル方式かインタプリタ方式かは言語仕様だけで一意に決まるものではなく、実際にどちらの方式で実行されるかは処理系（実装）にも依存する。例えば CPython は Python コードを通常いったんバイトコードにコンパイルしてから実行し、JavaScript の V8 エンジンはインタプリタと JIT コンパイラを組み合わせて実行し、C# も通常は中間言語へのコンパイルと実行時コンパイルを利用する。次の表は代表的な処理系・実行方法の例である。

| 実行方式 | 特徴 | 代表的な処理系・実行方法の例 |
|---|---|---|
| コンパイル方式 | 実行可能な形式を事前に生成する。動作は速いが開発中はその都度コンパイルが必要 | C、C#、Java、Swift |
| インタプリタ方式 | 記述したプログラムを実行時に逐次翻訳する。利用者による事前コンパイル操作が通常不要だが実行環境にインタプリタが必要 | JavaScript、PHP、Python、R、Ruby |
...
- **ホワイトボックステスト**: ソフトウェアの内部構造・関数の内容を見ながら、論理的な構造が正しいかを確認する。「全ての命令を少なくとも1回は実行する」**命令網羅**、「全ての分岐先（if の then/else など）を少なくとも1回は通る」**分岐網羅**、「各条件式を構成する基本条件それぞれの真偽の組み合わせを網羅する」**条件網羅**という、それぞれ異なる考え方がある（例えば `A and B` という条件式に (True, True) と (True, False) の2パターンを与えれば分岐網羅は満たせるが、A が False になるケースを試しておらず条件網羅までは満たさない）。
...
例えば合計値を求める関数で `total += num`（累積代入）と書くべきところを、誤って `total = num`（代入）と書いてしまうと、最後の要素しか返らないバグになる。
...
| XSS（クロスサイトスクリプティング） | 脆弱な Web サイトを通じて攻撃者が仕込んだコードを、そのサイトを信頼しているブラウザ上で、そのサイトの正規の文脈のまま実行させる。script タグの注入に限らず、属性値やイベントハンドラなど様々な経路で発生しうる |
| CSRF（クロスサイトリクエストフォージェリ） | 別サイトを閲覧中の被害者のブラウザに、Cookie など認証情報を自動的に付けたまま、ログイン済みの状態を悪用したリクエストを標的サイトへ送信させ、利用者が意図しない操作をサーバ側で実行させる |
...
ウォーターフォール型のシステム開発では、開発工程とテスト工程を対応させた**V字モデル**という考え方がある。「要求定義 ⇔ 総合テスト」「基本設計 ⇔ 結合テスト」「詳細設計 ⇔ 単体テスト」のように各設計工程が対応するテスト工程で検証される。ウォーターフォール型は「時間をかけて仕様を設計し、その通りに実装してテストする」方式で、仕様変更が難しい銀行系システムなどに従来から使われてきた。近年は、大まかに動く試作品を短期間で作り動作を確認しながら仕様を修正する**プロトタイプ型**や、機能を限定したシステムの試作・評価を反復しながら、各反復でリスクを分析してその低減策を講じつつ徐々に完成度を高めていく**スパイラル型**の開発も使われている。

### プログラムの改善・改良

- **構造化**: 「ここからここまでを繰り返す」「条件が成り立つときはこの処理を実行する」のように、章や節の見出しのように関数を定義し名前を付けることで、プログラム全体を把握しやすくする。
- **テスト駆動開発（TDD, Test Driven Development）**: 通常はプログラム作成後にテストを作るが、TDD ではまず小さなテストを1つ書いて失敗することを確認し（Red）、それを通す最小限の実装を行い（Green）、必要に応じてリファクタリングする（Refactor）、という短いサイクルを機能単位で繰り返しながら少しずつプログラムを作り上げていく。全てのテストを最初に一括で設計してから実装するわけではない。Python の標準ライブラリには `unittest` モジュールが用意されている。
