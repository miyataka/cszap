...
```mermaid
graph TD
  Q["中心的な問い<br/>OSは何のためにあるのか"]

  subgraph APP["アプリケーション層(用途特化ソフトウェア)"]
    A1["ブラウザ・エディタ・DBサーバ…"]
  end

  subgraph OSL["OS層(仲介者)"]
    MED["仲介: 抽象化・多重化・保護"]
    UNIV["普遍的な機能<br/>プロセス・インタフェース・永続化"]
    SPEC["拡張・特化機能<br/>組込み・サーバ種別・ブート"]
    DESIGN["設計課題とトレードオフ<br/>効率・堅牢性・移植性…"]
    SHELL["シェル・システムプログラミングへの露出"]
  end

  subgraph HWL["ハードウェア層(汎用)"]
    H1["CPU・メモリ・ディスク・NIC…"]
  end

  INFL["他分野からの影響<br/>ネットワーク・並列分散・マルチメディア"]
  SEC["セキュリティ/保護<br/>全層を横断する関心事"]
...
- 中央の「仲介: 抽象化・多重化・保護」がアプリ層とハードウェア層の間に立つ OS の役割 → [§1 仲介者としての OS](#os-as-mediator)。
- OS 層の内側は「普遍的な機能」「拡張・特化機能」「設計課題とトレードオフ」の3つ → [§2 普遍的な OS 機能](#universal-functions)・[§3 拡張・特化した OS 機能](#specialized-functions)・[§4 設計課題とトレードオフ](#design-issues)。
- 他分野からの影響は設計課題への入力として描いた → [§5 他分野からの影響](#influences)。
- シェル・システムプログラミングは普遍的な機能がユーザ・開発者に露出したもの → [§7 シェルとシステムプログラミングへの露出](#shells-sysprog)。
- セキュリティ/保護は全層を横断する破線で表現した → [§6 横断的関心としてのセキュリティ/保護](#security-protection)。実装の具体例（保護リング等）は [OS-Protection §6](OS-Protection.md#rings) で扱う。
