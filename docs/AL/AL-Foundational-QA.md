...
??? question "Q14. Floyd–Warshall ってどんなアルゴリズム？"
    ...
    **A.** **全頂点対間の最短経路 (all-pairs shortest paths)** を一気に求める動的計画法。「どの頂点からどの頂点へも」の最短距離を V×V の表に埋める。**負の辺があってもOK**（負閉路がなければ）。[ダイクストラ](#q12)が「1始点→全頂点」なのに対し、こちらは「全始点×全終点」。
    ...
    ### 核心：「経由してよい中継点」を1つずつ増やす

    キーは **中間頂点 (intermediate vertex)** の考え方。

    > `dist[i][j]` を「**頂点 `1..k` だけを中継点に使ってよい**ときの i→j 最短距離」と定義し、使える中継点 `k` を1個ずつ増やす。

    中継点 `k` を新しく加えると、i→j の経路は2択：

    1. **k を経由しない**：今までの `dist[i][j]`（`k-1` までの最短）のまま
    2. **k を経由する**：`dist[i][k] + dist[k][j]`
    ...
    `A→B(3), A→C(8), B→C(2)` で中継点 k=B を解禁すると、A→C は 8 から **A→B→C = 3+2 = 5** に更新される。中継点を全部回すと全ペアの最短が埋まる。
