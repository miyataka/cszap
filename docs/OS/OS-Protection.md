...
!!! note "Meltdown/Spectre のしくみ——ウェイターと厨房の比喩"
    レストランのウェイター（CPU）が、客（分岐）の注文を確定する前に「たぶんこれだろう」と**先読み**して厨房（キャッシュ）に伝えてしまう（＝投機実行）。予想が外れれば注文は取り消され客には届かない——しかし厨房の棚には**先読みで一瞬取り出した食材の「隙間」という痕跡**が残る。誰かがその隙間の有無を**タイミング計測**で調べれば、注文の中身（本来届かないはずのカーネルメモリ）が漏れる。

    ```mermaid
    sequenceDiagram
        participant 客 as 客(未確定の分岐)
        participant CPU as ウェイター(CPU/投機実行)
        participant 棚 as 厨房の棚(キャッシュ)
        participant 観測者 as 観測者(攻撃者)

        客->>CPU: 「たぶんこれを頼むだろう」と先読み
        CPU->>棚: 投機的に食材を取り出す(カーネルメモリに触れる)
        Note over CPU: 予想が外れ、注文は取り消される(実行結果は破棄)
        Note over 棚: だが棚の「隙間」(キャッシュの状態)は元に戻らない
        観測者->>棚: 隙間の有無をタイミング計測で調べる
        棚-->>観測者: アクセス時間の差から中身が漏れる
    ```

    緩和策 **KPTI** は投機実行そのものを止めるのではなく、**ページテーブルを分離**してユーザモードから見えるカーネル空間の実体をほぼゼロにすることで、先読みしても盗める食材（カーネルメモリの実体）自体を厨房から遠ざける対策である。

    ```mermaid
    graph LR
      subgraph "KPTI 適用前"
        U1[ユーザ側ページテーブル] -.カーネル空間の一部が見える.-> K1[カーネルメモリ]
      end
      subgraph "KPTI 適用後"
        U2[ユーザ側ページテーブル] -->|カーネルのマッピングをほぼ除去| X["投機的に辿っても<br/>実体が無い"]
        K2["カーネル用ページテーブル<br/>(システムコール時にのみ切替)"]
      end
    ```
