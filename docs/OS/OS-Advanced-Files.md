...
- **検索 (searching)**: パス階層をたどる線形探索はファイル数が増えると遅い。**ディレクトリの内部構造**（線形リストではなくB木やハッシュテーブル。→ [AL-Foundational §3](../AL/AL-Foundational.md#tree) の平衡木・[§2](../AL/AL-Foundational.md#assoc-set) のハッシュ表）や、**索引データベース**（`locate` のインデックス、Spotlight のようなデスクトップ検索）で高速化する。
