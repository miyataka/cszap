...
- **テキスト**: 文字ごとに固有の番号（文字コード）を割り当てて2進法のデータとして扱う。アルファベットや数字のように種類が少ない文字は7bit、漢字のように種類が多い文字は16bitで表現する。
- **画像**: 大きく分けて**ラスタデータ**（色情報を持つ点＝画素の集合。ビットマップデータとも呼ぶ。写真・自然画向き。拡大・縮小・変形すると画質が粗くなる）と**ベクタデータ**（点とそれを結ぶ線や面を数式・計算処理で表現。イラスト・図面向き。拡大・縮小・変形しても画質が維持される）がある。画像の細かさを示す**解像度**は1インチあたりの点の数（dpi、または ppi）で表され、値が高いほど滑らかになる。色の表現には、光の三原色（赤・緑・青）を組み合わせる**加法混色**（RGBカラー、赤+緑+青=白）と、塗料の三原色（シアン・マゼンタ・イエロー）を組み合わせる**減法混色**（CMYKカラー、シアン+マゼンタ+イエロー=黒。印刷では輪郭を表すK（Key Plate、黒）を加えることが多い）がある。ディスプレイなどではRGB各色8bit×3で24bit、約1,677万色（2の24乗）を表現できる。
- **動画**: 静止画（フレーム）を短い時間間隔で連続表示し、目の残像効果を利用して動いているように見せたもの。1秒あたりのフレーム数を**フレームレート**という。保存時にはデータ量を抑えるため圧縮（コーデック）を行うことが多く、再生環境が対応するコーデックを持たないと再生できない場合がある。
...
**誰から誰へ**: コミュニケーションの語源はラテン語 communis（共通の）＋ munitare（通行可能にする）で、「共通して行き交う」ことを意味する。コミュニケーションが成り立つには、まず「送り手」と「受け手」が存在する必要がある。両者は同じ文脈（コンテクスト）を共有しているとは限らないため、送り手はコンテクストを**記号化（encoding）**し、受け手はそれを**解読（decoding）**する。正しく変換するためには、両者の間で**コード体系**（言語＝自然言語・人工言語、視覚言語＝色・形・シンボル・手話など、非言語＝身振り・表情）が共有されていなければならない。コード体系を共有していても、文脈や解釈によって意味がずれることは避けられないため、相互に調整しながらコミュニケーションを構築していく必要がある。

**何を**: コンテンツ（伝える内容）はメディア（媒介するもの）に対応する中身を指す。自分で探し生み出した**一次情報**と、誰かが生み出した情報を再利用した**二次情報**があり、同様に一次コンテンツ・二次コンテンツにも分類される。誰かが生み出した創造物を使うだけでなく、自分の創造性を発揮して独自性を生み出すことが大事だとされる。
...
- **ラスタ系画像処理ソフト**: 画像を小さな点（ピクセル）で描画する。拡大や変形をすると画像が粗くなる場合があり、ベクタ系と異なり画像内の一部の円や長方形だけを移動・削除することはできない。デジタルカメラで撮影した画像も、ラスタ系画像処理ソフトで作成した画像と同様にピクセルの集まりでできている。
...
**④ プロトタイプを作成する**: プロトタイプは完成品の原型ではなく、デザインを検討していく過程で作成されるものである。完成品に近い1個を時間をかけて作るのではなく、簡単に作成できるものを多数作る。Webページやアプリケーションのユーザインタフェースの検討では、画面のレイアウトだけでなく利用者の操作による画面遷移の様子も紙の上に再現する**ペーパープロトタイピング**という手法が用いられる。紙とペンだけで作成できるため導入は簡単だが、画面遷移の再現には紙の差し替えが必要で実際の操作感をイメージしにくいところがあり、最近ではペーパープロトタイピングの画面レイアウトを取り込んで画面遷移を情報端末上で再現するプロトタイピングツールも利用されている。
