...
- **リングバッファ (ring buffer / circular buffer)**: 固定長の配列を輪のように使い、**書き込み位置**と**読み出し位置**の2つのポインタで管理する。バッファがいっぱいになったら先頭に戻って上書きせずに待つ（あるいは古いデータを捨てる）。生産者（デバイス）と消費者（OS/アプリ）の速度差を吸収する定番構造で、§3 の DMA 通信プロトコルの主役でもある。
