...
!!! info "使い方"
    - 採点記録を追加したら、**弱点一覧**に行を足す（1弱点 = 1行。採点 0.5 以下の設問と、講評で「積み残し」と書かれた項目が対象）。
    - 再テストで改善したら、その行の「再テスト」と「状態」を更新する。状態は **未対処 / 改善 / 克服** の3つ。
...
採点 0.5 以下の設問と、講評が「積み残し」「最優先」と名指しした項目を、初出の古い順に並べる。**状態**は再テストの結果で決める——再テストで 1.0 なら **克服**、点が上がったなら **改善**、下がった・変わらない・未実施なら **未対処**。
...
| 4 | Q10 隣接リスト vs 隣接行列の使い分け——初回は答えられたのに再テストで「わからない」＝**忘却**。講評が「最優先で再記憶」と名指し | AL-Foundational | 2026-06-09 | 0.7 | 0.0 | 未対処 | [2026-06-09](AL/AL-Foundational-Quiz.md#2026-06-09-26) / [2026-06-14](AL/AL-Foundational-Quiz.md#2026-06-14-2026-06-09-26) |
...
| 9 | Q19 ダイクストラ法の説明——適用条件を「無向」と誤認（正は**非負の重み**で有向/無向・閉路は不問）。講評が「積み残し・次回の最優先」と名指し | AL-Foundational | 2026-06-09 | 0.2 | 0.3 | 改善 | [2026-06-09](AL/AL-Foundational-Quiz.md#2026-06-09-26) / [2026-06-14](AL/AL-Foundational-Quiz.md#2026-06-14-2026-06-09-26) |
...
| 11 | Q21 トポロジカルソートの説明——「どのように」（Kahn 法／DFS 帰りがけ順の逆）が無い | AL-Foundational | 2026-06-09 | 0.4 | 0.5 | 改善 | [2026-06-09](AL/AL-Foundational-Quiz.md#2026-06-09-26) / [2026-06-14](AL/AL-Foundational-Quiz.md#2026-06-14-2026-06-09-26) |
...
| 31 | Q12 → T7 Dirty COW（COW の競合状態がそのまま権限昇格になる）——未回答。再テストでも「タイミングのスキ」という機構と、共有フレーム経由で被害が全プロセスに波及する規模への言及が無い | OS-Memory | 2026-07-27 | 0.0 | 0.5 | 改善 | [2026-07-27](OS/OS-Memory-Quiz.md#2026-07-27-os-protection-os-process-os-memory-12-q912) / [2026-07-28](OS/OS-Memory-Quiz.md#2026-07-28-tlbcow) |
...
!!! info "状態の内訳"
    全40件のうち **克服** 3件（#24・#27・#29）／**改善** 11件／**未対処** 26件。改善で止まっている 11 件は「点は上がったが満点ではない」状態なので、克服まで運ぶ余地がまだある。
