...
このユニットは大きく **2 段の問い**に分かれる。まず「そもそも解けるのか」を確かめ、解ける（扱いやすい）と分かったものについて初めて「どれだけ速く解けるか」を測る。

```mermaid
flowchart TD
  Q0["アルゴリズムを評価する: 何を測るか?"]
  Q0 --> Q1["問い1: そもそも解けるか?<br/>扱いやすさ"]

  subgraph TRACT["扱いやすさ Tractability"]
    T1["P: 多項式時間で解ける"] --> T2["NP: 検証は多項式時間"] --> T3["NP完全: NPの中で最難"]
    T3 --> T4["帰着<br/>難しさ・易しさを移す"]
  end
  Q1 --> T1

  T3 --> Q2["問い2: どれだけ速く?<br/>漸近解析"]

  subgraph ASYM["漸近解析"]
    A1["枠組み<br/>best/avg/worst"] --> A2["漸近記法<br/>O・Ω・Θ"] --> A3["計算量クラス<br/>O(1)…O(n!)"]
    A3 --> A4["時間と空間の<br/>トレードオフ"] --> A5[経験的測定]
  end
  Q2 --> A1

  A3 -.->|より精密に| KA1["KA Core:<br/>little-o/ω/θ・漸化式<br/>償却解析・チューリング機械モデル"]
```

**図の読み方**

- 中心の問い: 「このアルゴリズム、何を測ればよいか?」。
- 問い1（そもそも解けるか）→ [§6 扱いやすさ](#tractability)。P・NP・NP完全は[§6a](#p-np-npc)、帰着も[§6c](#tractability)（同じ節内）。
- 問い2（どれだけ速く）→ 枠組みは[§1](#framework)、漸近記法は[§2](#asymptotic-notation)、計算量クラスは[§3](#complexity-classes)、時間と空間のトレードオフは[§5](#space-time-tradeoffs)、経験的測定は[§4](#empirical)。
- KA Core の4節（little-o/ω/θ・漸化式・償却解析・チューリング機械モデル）は、この2つの問いを**より精密にする補助ツール** → [§7](#little-notations)・[§8](#recurrences)・[§9](#amortized)・[§10](#tm-complexity)。
- 見積もりの具体的な手順は[実戦: アルゴリズムの計算量を見積もる思考フロー](#estimate-flow)へ。次ユニット [AL-Models §2](AL-Models.md#chomsky-hierarchy) は「そもそも解けるか」をさらに深掘りし、決定不能性まで踏み込む。
...
## 2. 漸近記法 ＝「関数をどう抑えるか」 {#asymptotic-notation}
...
## 7. little-o / little-ω / little-θ {#little-notations}
