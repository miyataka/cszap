...
```mermaid
flowchart TD
  Q["中心の問い<br/>コンピュータは情報をどう処理するか"]

  EXT1["2 外部装置との接続<br/>センサ"]
  EXT2["2 外部装置との接続<br/>アクチュエータ"]

  IN["入力装置"]

  subgraph CORE["処理"]
    direction TB
    CPU["1 コンピュータの仕組み<br/>CPU・メモリ、論理演算"] --> CTRL["3 制御構造<br/>順次・分岐・反復"]
    CTRL --> APP["4 応用的プログラム<br/>リスト・関数・WebAPI"]
    APP --> ALGO["5 アルゴリズムの比較<br/>探索・ソート"]
  end
  ...
  ALGO -.->|何を計算するか| MODEL["6 モデル化と<br/>シミュレーション"]
...
- 中心の問い「コンピュータは情報をどう処理するか」を軸に、入力から出力までの流れを追う。
- 入力装置・出力装置はどちらも [1. コンピュータの仕組み](#computer-structure) の構成要素に対応し、センサ・アクチュエータを介した [2. 外部装置との接続](#external-devices) が入出力の両端につながる。
- 「処理」の内部は [1. コンピュータの仕組み](#computer-structure)（CPU・メモリ、論理演算・四則演算・計算誤差）→ [3. 基本的プログラム（制御構造）](#control-structures) → [4. 応用的プログラム](#applied-programs) → [5. アルゴリズムの比較](#algorithm-comparison) の順に細分化される。
- [6. モデル化とシミュレーション](#modeling-simulation) は、アルゴリズムを使って「何を計算するか」を現実の現象に応用する節として、処理から分岐させて横に置いた。
- 探索・ソートの比較は、CS2023側の [AL-Foundational §4](../AL/AL-Foundational.md#search)・[§5](../AL/AL-Foundational.md#sort) と重なる領域。
...
コンピュータで変数は、指定されたビット数の「箱」に入っていると考えるとわかりやすい。この箱に収まるものであれば誤差は生じないが、収まらないものは誤差が生じる。ただし「収まらない」には、値そのものが箱の表現できる範囲を超えてしまう**オーバーフロー**と、範囲内ではあるが必要な桁数（精度）が箱の大きさを超えるために表現可能な最も近い値に**丸められる**ことで生じる**丸め誤差**の2種類があり、区別して考える必要がある。
...
print(x)                 # 画面にxの値を表示
x = 1.797693134862315799999e+308  # float型の最大値の小数点以下に5桁「9」を追加した値
print(x)                 # 画面にxの値を表示
x = 1.8e+308              # float型の最大値より大きな値
print(x)                 # 画面にxの値を表示
```

実行結果は `1.7976931348623157e+308` → `1.7976931348623157e+308`（追加した桁は表現可能な精度を超えるため、最も近い値である元のfloat型の最大値に丸められる＝丸め誤差）→ `inf`（float型で表現できる範囲そのものを超えたことによるオーバーフロー）となる。
...
**デジタル型**のセンサ・アクチュエータはデジタル信号（0と1の組み合わせであるビット列）で入出力するものであり、入出力ポートに直接つながる。二値（オン／オフ）で動作するものだけでなく、デジタル温度センサのように多段階の測定値をビット列として出力する機器も含まれる。一方、連続的な物理量をそのまま扱う**アナログ型**の場合は、アナログをデジタルに変換する **AD コンバータ**や、デジタルをアナログに変換する **DA コンバータ**を経由する構成になる。
...
（上図はデジタル型・アナログ型それぞれの入出力における変換経路を示す概念図であり、信号が何段階の値を取りうるかを表すものではない。）
...
while True:                          # ずっと繰り返す
    reading = accelerometer.get_x()  # 加速度センサのX軸方向の加速度（単位: ミリg）を読む
    if reading > 100:                # X軸方向の加速度が100（ミリg）より大きいなら
        display.show("R")            # LEDに「R」を表示
    else:                            # そうでなければ
        display.show("-")            # LEDに「-」を表示
```

`get_x()` が返すのは傾斜角そのものではなく、X軸方向の**加速度**（ミリg単位）である。静止しているときはこの値から重力成分の向きをもとに傾きを推測できるが、移動しているときは運動によって生じる加速度も加わるため、必ずしも傾きだけを表すとは限らない点に注意が必要である。
...
ある範囲内で、特定の分布に従って出現する数を**乱数**という。範囲内のすべての値が同じ確率で現れる**一様乱数**が代表例だが、正規分布のように値によって出現確率が異なる**非一様な乱数**もある。また、コンピュータが計算だけで規則的に生成する乱数は正確には**擬似乱数**と呼ばれるが、コンピュータは熱雑音などの物理的なエントロピー源を利用して真の乱数（真性乱数）を得られる場合もある。ここでは、乱数の中でも**一様な擬似乱数**を扱う。乱数をプログラム内で用いることで、実行するたびに処理内容が変わる変則的なプログラムを作成でき、現実世界のシミュレーションなどに活用できる。
...
- **二分探索**: リストの中から探索範囲を半分ずつ狭めながら目的のデータを探し出す方法。あらかじめ昇順（または降順）にソートされたリストが対象になる。下限添字・上限添字から中央添字を求め、中央値と探索値を比較して探索範囲を半分に絞り込む処理を繰り返す。
    ...
    上記のコード例は、リスト `a` があらかじめ**昇順**にソートされていることを前提としている（降順にソートされたリストを対象にする場合は、大小比較の向きを逆にする必要がある）。
...
- **選択ソート**: リスト内のデータから最小値を探索し、最小値から順に取り出すことで並べ替えを実現する。
    ...
    上記のコード例は、未整列部分の中で現在の最小候補より小さい値が見つかるたびにその都度交換する**選択ソートの変種**である。標準的な選択ソートでは、未整列部分から最小値の位置を探し切ってから、その位置と先頭要素を最後に1回だけ交換する。どちらの手順でも並べ替えの結果自体は正しい。

- **クイックソート**: リスト内の1つのデータを軸として大小2つに分割したあと、分割したデータに対して同じ処理を再度行うことで並べ替えを実現する。

- **比較**: データ数がN個の場合、選択ソートは「複数の要素から最小値を探す」処理をおよそN-1回、しかもその都度未整列部分の要素すべてと比較するため、比較回数はデータ数の増加に対して `Θ(N²)` で増える。クイックソートは基準値（ピボット）をもとに2分割しながら並べ替えの処理を行うが、二分探索が片側だけを調べればよいのに対し、クイックソートは分割した両側を整列していく必要があるため増加のしかたは異なり、平均的な比較回数は `Θ(N log N)` になる一方、最悪の場合は選択ソートと同じ `Θ(N²)` になりうる。逆順に並べ替えられている場合が必ず悪条件になるわけではなく、これはピボットの選び方（先頭・末尾・中央・ランダムなど）に依存する。ソートアルゴリズムの場合においても、必ずしもクイックソートの方が優れたアルゴリズムであるとは言い切れない。
...
対象の特性による分類では、静的モデルはプラスチックモデルや建築図面などのように時間的要素を含まないもの、動的モデルはレジの待ち行列や気象予測、生物の成長など時間的要素を含んだ事象のモデルである。教材ではこれをふまえ、動的モデルをさらに確定モデル（不規則な現象を含まず方程式などで表せるモデル）と確率モデル（サイコロやくじ引きのような不規則な現象を含んだモデル）に分けて説明している。ただし確定的／確率的という区別は本来、静的モデルにも当てはまりうる別の分類軸であり（例えばサイコロの出目の確率分布のように、時間変化を伴わない静的な確率モデルも存在する）、上図は教材の説明に沿った一例であって、確定・確率を動的モデルだけの下位分類と厳密に位置づけられるわけではない点に注意したい。
...
点の総数を増やすと、結果のばらつき（分散）が小さくなり、確率的に円周率の真の値へ収束していく（大数の法則）。ただし、試行数を増やしても常に単調に真の値へ近づくとは限らず、一時的に真の値から遠ざかる場合もある点には注意が必要である。
...
**生命体の増加シミュレーション**では、自然界における生物の個体数がある一定の環境内で一気に繁殖する性質をモデル化する。ここでは一定の時間刻みごとに個体数を更新する**離散時間モデル**として考え、個体数が増加する数は「個体数 × 増加率」、個体が減少する数は「個体数 × (現在の個体数 / 環境収容力) × 増加率」と定義する（`N_次 = N + rN(1 - N/K)` に相当するロジスティック方程式の離散化）。生存しうる個体数の上限（**環境収容力**）を超えて増えようとすると減少数が増加数を上回るように設計されているため、多くの条件では環境収容力に収束する**S字曲線（ロジスティック曲線）**に近い挙動を示す。ただし離散時間モデルであるため、初期値・増加率・時間刻みの設定によっては単調に収束せず、振動したり周期的に変動したりする場合もある点に注意が必要である。

**ランダムウォークのシミュレーション**は、ブラウン運動や株価の変動など不確実な現象を表すモデルとして用いられる。ある物体が次の時間変化後にどの方向へ行くか予測できない状況をモデル化する例として、x軸・y軸それぞれの増分が独立に一様な乱数に従う場合、現在位置 `(x0, y0)` と次の時間変化後の位置 `(x, y)` の関係は、0〜1の乱数を発生させる関数 `random()` を用いて次のように表せる。
...
ただし、この方法はx・yそれぞれの増分（-0.5〜0.5の範囲）が一様に分布するのであって、移動する**方向（角度）**が一様に分布するわけではない（対角方向にやや進みやすくなる偏りが生じる）。移動する向きを一様にしたい場合は、角度 `θ = 2π × random()` を発生させ、移動距離を `s` として `Δx = s cos θ`、`Δy = s sin θ` のように極座標で増分を計算する方法が使われる。
