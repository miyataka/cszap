...
```mermaid
graph TD
  Q["中心的な問い：物理ブロックの集まりを、<br/>名前を持つファイルの階層としてどう見せるか"]
  MOUNT["入口：マウント・アクセス制御・共有<br/>複数のFSを1つの名前空間へ接ぐ"]
  NS["名前空間<br/>ディレクトリ構造とパス解決"]
  FILE["ファイル概念<br/>データ・メタデータ・操作・アクセスモード"]
  ALLOC["割り当て方式<br/>連続・連結・FAT・索引"]
  FREE["空き領域管理<br/>ビットテーブル vs リンク方式"]
  BLOCKS["ディスクブロック（物理記憶）"]
  MEMNOTE["注記：割り当て問題はメモリと同型<br/>対象がRAMからディスクに変わっただけ"]

  Q -.-> MOUNT
  MOUNT --> NS
  NS -->|"パスをたどりinodeへ"| FILE
  FILE -->|"データブロックへのポインタ"| ALLOC
  ALLOC -->|"空きブロックを要求"| FREE
  FREE --> BLOCKS
  MEMNOTE -.-> ALLOC
  MEMNOTE -.-> FREE
...
- 「入口：マウント・アクセス制御・共有」（複数のFSを1つの名前空間へ接ぐ）は [§2 マウント・アクセス制御・共有](#mount-access-sharing)。
- 「名前空間」（ディレクトリ構造とパス解決）は [§4 ディレクトリ構造とファイルの特定](#directory-structures)。
- 「ファイル概念」（データ・メタデータ・操作・アクセスモード）は [§1 ファイルの概念](#file-concept)。
- 「割り当て方式」（連続・連結・FAT・索引）は [§3 基本的な割り当て方式](#allocation-methods)。
- 「空き領域管理」（ビットテーブル vs リンク方式）は [§5 空き領域管理](#free-space-management)。どちらを選ぶかは「割り当て方式」の選択と表裏一体（本文§5参照）。
- 「注記：割り当て問題はメモリと同型」が示すとおり、割り当て方式と空き領域管理の設計問題は [OS-Memory §5 アロケータ](OS-Memory.md#allocators) と同型——対象がRAMのバイト範囲からディスクのブロックに変わっただけ。
...
| **連結割り当て (linked allocation)** | 各ブロックが**次のブロックへのポインタ**を持ち、鎖のようにつながる（→ [AL-Foundational §1](../AL/AL-Foundational.md#linear) の連結リストがそのままディスク上に置かれた形） | 外部断片化が起きない。空きブロックをどこからでも継ぎ足せる。だが**ランダムアクセスが遅い**（n 番目のブロックを読むには先頭から n 回辿る）うえ、ポインタ分の記憶域を消費し、途中のポインタが壊れると鎖全体が読めなくなる（信頼性の弱さ） |
...
- **非巡回グラフ (acyclic graph)**: 同じファイル（や部分木）を複数の場所から指せる**リンク**（ハードリンク・シンボリックリンク）を許す。共有はできるが、循環参照が起きない範囲に限る（→ [AL-Foundational §3](../AL/AL-Foundational.md#tree)）。
