...
- 全域木 (spanning tree) という概念そのものは [AL-Foundational §6](../AL/AL-Foundational.md#graph-algorithms) のグラフアルゴリズム——最小全域木 (MST) を求める Prim 法・Kruskal 法——で扱ったもの。ただし STP が作るのは重み最小の木ではなく、**ルートブリッジからのコストが最小になる木**（最短経路木に近い）である点が異なる。そして最大の違いは、**全体を見渡す計算主体が存在せず、各スイッチが隣とメッセージを交換するだけで木が決まる**こと——**アルゴリズムの授業で1台の計算機の中で解いた問題を、分散環境で解き直している**のが STP の面白いところ。
